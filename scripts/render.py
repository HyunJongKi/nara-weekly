...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* 소스 배지 - 어느 사이트에서 왔는지 즉시 보이도록 서로 다른 톤 */
  .chip.source { font-weight:700; border:1px solid transparent; }
  .chip.source.src-nara    { background:#e0f2fe; color:#075985; border-color:#7dd3fc; }
  .chip.source.src-iris    { background:#ede9fe; color:#5b21b6; border-color:#c4b5fd; }
  .chip.source.src-bizinfo { background:#fef3c7; color:#92400e; border-color:#fcd34d; }
  .chip.source.src-kofpi   { background:#dcfce7; color:#166534; border-color:#86efac; }
  ...
  <main class="main">
    <header class="page-head">
      <div>
        ...
        <div class="meta" id="page-sub">🏛️ 나라장터 + 🔬 IRIS + 💼 기업마당 + 🌲 임업진흥원 통합 발주정보</div>
      </div>
    ...
    <div class="filters" id="filter-bar">
      ...
      <select id="type">
        ...
        <option value="order_plan">🏛️ 발주계획</option>
        <option value="pre_spec">🏛️ 사전규격</option>
        <option value="iris_rnd">🔬 IRIS 사업공고</option>
        <option value="bizinfo_support">💼 기업마당 지원</option>
        <option value="kofpi_bid">🌲 임업진흥원 공모</option>
      </select>
      <select id="source">
        <option value="">전체 출처</option>
        <option value="nara">🏛️ 나라장터</option>
        <option value="iris">🔬 IRIS</option>
        <option value="bizinfo">💼 기업마당</option>
        <option value="kofpi">🌲 임업진흥원</option>
      </select>
    ...
    <section data-view-pane="dashboard">
      ...
      <div class="footnote">※ 본 통계는 🏛️ 나라장터(발주계획·사전규격) + 🔬 IRIS(범부처 R&D 사업공고) + 💼 기업마당(중소벤처 지원사업) + 🌲 임업진흥원 공고를 통합한 것으로, 3개 외부 소스는 나라장터와 중복되지 않는 항목만 표시됩니다. 실제 계약과는 차이가 있을 수 있습니다.</div>
    </section>
...
const TYPE_LABEL = {
  ...
  pre_spec:        "사전규격",
  iris_rnd:        "IRIS 사업공고",
  bizinfo_support: "기업마당 지원",
  kofpi_bid:       "임업진흥원 공모",
};
// 출처별 배지 - 사이트가 어디인지 즉시 인식되도록 카드 상단에 표시.
const SOURCE_META = {
  order_plan:      { label: "🏛️ 나라장터",   cls: "src-nara"    },
  pre_spec:        { label: "🏛️ 나라장터",   cls: "src-nara"    },
  iris_rnd:        { label: "🔬 IRIS",        cls: "src-iris"    },
  bizinfo_support: { label: "💼 기업마당",    cls: "src-bizinfo" },
  kofpi_bid:       { label: "🌲 임업진흥원",  cls: "src-kofpi"   },
};
function sourceBadge(src) {
  const m = SOURCE_META[src] || { label: src || "기타", cls: "src-etc" };
  return `<span class="chip source ${m.cls}">${m.label}</span>`;
}
...
function getFilters() {
  return {
    ...
    src: (document.getElementById("source") || {value:""}).value,
...
// source_type → source group (필터 매핑)
function sourceGroup(st) {
  if (st === "order_plan" || st === "pre_spec") return "nara";
  if (st === "iris_rnd") return "iris";
  if (st === "bizinfo_support") return "bizinfo";
  if (st === "kofpi_bid") return "kofpi";
  return st;
}
...
function applyFilter(items, f) {
  return items.filter(it => {
    ...
    if (f.src && sourceGroup(it.source_type) !== f.src) return false;
...
function aggregate(items) {
  const byDate = {}, byType = {order_plan:0, pre_spec:0, iris_rnd:0, bizinfo_support:0, kofpi_bid:0};
  const bySource = {nara:0, iris:0, bizinfo:0, kofpi:0};
  const byBudget = {}, byRegion = {}, byAgency = {}, byAgencyType = {}, byKw = {};
  ...
  for (const it of items) {
    ...
    const grp = sourceGroup(it.source_type);
    bySource[grp] = (bySource[grp]||0) + 1;
  ...
  return { byDate, byType, bySource, byBudget, byRegion, byAgency, byAgencyType, byKw, byKwType, byRegionType,
...
function renderAll() {
  ...
  const srcSubParts = [];
  if (A.bySource.nara)     srcSubParts.push(`🏛️ 나라장터 ${A.bySource.nara}`);
  if (A.bySource.iris)     srcSubParts.push(`🔬 IRIS ${A.bySource.iris}`);
  if (A.bySource.bizinfo)  srcSubParts.push(`💼 기업마당 ${A.bySource.bizinfo}`);
  if (A.bySource.kofpi)    srcSubParts.push(`🌲 임업진흥원 ${A.bySource.kofpi}`);
  const srcSub = srcSubParts.join(" · ") || "-";

  document.getElementById("kpis").innerHTML = [
    {label:"총 공고건수", value: items.length.toLocaleString(), sub:`발주계획 ${A.byType.order_plan||0} · 사전규격 ${A.byType.pre_spec||0}`},
    {label:"🌐 출처별", value: (A.bySource.nara||0)+(A.bySource.iris||0)+(A.bySource.bizinfo||0)+(A.bySource.kofpi||0), sub: srcSub},
  ...
  if (CURRENT_VIEW === "dashboard") {
    ...
    // 도넛 - 5가지 소스 (값이 0이면 자동 제외)
    (function() {
      const rows = [
        ["🏛️ 발주계획", A.byType.order_plan],
        ["🏛️ 사전규격", A.byType.pre_spec],
        ["🔬 IRIS",        A.byType.iris_rnd],
        ["💼 기업마당",    A.byType.bizinfo_support],
        ["🌲 임업진흥원",  A.byType.kofpi_bid],
      ].filter(([, v]) => (v||0) > 0);
      doughnutChart("ch-type", rows.map(r=>r[0]), rows.map(r=>r[1]));
    })();
...
function downloadXlsx() {
  ...
  const rows = sortedItems.map(it => {
    ...
    const row = {
      "⭐ 찜": isFav(it) ? "★" : "",
      "출처": (SOURCE_META[it.source_type]||{label:""}).label,
...
function setView(name) {
  ...
  const subs = {
    dashboard:"🏛️ 나라장터 + 🔬 IRIS + 💼 기업마당 + 🌲 임업진흥원 통합 발주정보",
...
function initUI() {
  ...
  ["q","type","source","kw","region","agency_type","sort"].forEach(id => {
    const el = document.getElementById(id); if (el) el.addEventListener("input", renderAll);
  });
  document.getElementById("reset").addEventListener("click", () => {
    ["q","type","source","kw","region","agency_type"].forEach(id => {
