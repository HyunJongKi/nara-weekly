...
# A6(R&D·기술개발)·A7(설계·규격)·A9(안전·환경조사) 관련 표현은 STRONG 에서 제외.
RESEARCH_STRONG = re.compile(
    r"(연구\s*용역|학술\s*연구|학술용역|조사\s*용역|조사연구|기초\s*연구|기획\s*연구|"
    r"정책\s*연구|타당성\s*(조사|분석|연구|검토)|실태\s*조사|모니터링|"
    r"마스터플랜|기본\s*계획\s*수립|중장기\s*계획|성과\s*평가|성과평가|"
    r"역량강화|용역\s*연구|시범\s*사업|평가\s*연구|"
    r"실무활용\s*정립|정립\s*연구|중심지\s*활성화|"
    r"공간\s*정비\s*사업|공간정비|재구조화|기술\s*수요|수요\s*분석|기획\s*용역|"
    r"제도\s*개선|법제|개정\s*방향|개선\s*방안|시행계획|재생\s*기본|"
    r"컨설팅|자문)"
)

# 하드 제외 - 무조건 배제. bsnsDivNm 카테고리와 제목에서 이 표현이 나오면 용역 아님.
# A6(R&D·기술개발), A7(설계·규격), A9(안전·환경조사) 카테고리도 사용자 요청으로 하드 제외.
RESEARCH_HARD_EXCLUDE = re.compile(
    ...
    # A6 (R&D·기술개발) 제외
    r"연구\s*개발|연구개발|R&D|R\s?&\s?D|실증\s*연구|실증연구|"
    r"기술\s*개발|기술개발|시제품|육종\s*(연구|사업)?|품종\s*(개발|육성)|"
    r"마커\s*(개발|선발)|시험\s*(연구|사업|개발)|장비\s*(개발|도입)|기술\s*이전|"
    # A7 (설계·규격) 제외
    r"실시\s*설계|기본\s*설계|설계\s*기준|설계\s*용역|설계용역|"
    r"규격서\s*작성|성과품\s*제작|"
    # A9 (안전·환경조사·지질조사) 제외
    r"안전\s*점검|안전점검|정밀\s*안전|정밀안전|안전\s*진단|안전진단|"
    r"환경\s*영향\s*평가|환경영향\s*평가|환경영향평가|"
    r"지하수\s*영향\s*조사|지하수영향조사|지하수영향|"
    r"지질\s*조사|지질조사|시추\s*조사|시추조사|측량\s*용역|측량용역|"
    r"영향\s*조사|영향조사|암반\s*관정|"
    # 감리·감독
    r"감리\s*용역|감리용역|건설사업관리)"
)
